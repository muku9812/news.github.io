 ...
 * @package News-Get
 */
if ( ! function_exists( 'newsget_enqueue_styles' ) ) :
	/**
	 * @since 0.1
	 */
	function newsget_enqueue_styles() {
		wp_enqueue_style( 'newsup-style-parent', get_template_directory_uri() . '/style.css' );
		wp_enqueue_style( 'newsget-style', get_stylesheet_directory_uri() . '/style.css', array( 'newsup-style-parent' ), '1.0' );
		wp_enqueue_style('bootstrap', get_template_directory_uri() . '/css/bootstrap.css');
		wp_dequeue_style( 'newsup-default',get_template_directory_uri() .'/css/colors/default.css');
		wp_enqueue_style( 'newsget-default-css', get_stylesheet_directory_uri()."/css/colors/default.css" );
		if(is_rtl()){
...
add_action( 'wp_enqueue_scripts', 'newsget_enqueue_styles', 9999 );


add_action( 'customize_register', 'newsget_customizer_rid_values', 1000 );
function newsget_customizer_rid_values($wp_customize) {
...
function newsget_theme_setup() {

//Load text domain for translation-ready
load_theme_textdomain('newsget', get_stylesheet_directory() . '/languages');
...
add_action( 'after_setup_theme', 'newsget_theme_setup' );
 ...
 */
function newsget_widgets_init() {
	

	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Canvas Section', 'news-get'),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Left Sidebar Section', 'news-get'),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Right Sidebar Section', 'news-get'),
...
add_action( 'widgets_init', 'newsget_widgets_init' );


function newsget_remove_some_widgets(){
// Unregister Frontpage sidebar
unregister_sidebar( 'front-page-sidebar' );
}
add_action( 'widgets_init', 'newsget_remove_some_widgets', 11 );

function newsget_menu(){ ?>
<script>
...
<?php }
add_action( 'wp_footer', 'newsget_menu' );	